...
\pdfinfo{
/Title (Using Counterfactual Regret Minimization in Gin Rummy)
/Author (Sarah Kettell, Jacob Gallucci, Richard Bowser, Christian Overton)
...
\title{Using Counterfactual Regret Minimization in Gin Rummy}
\author {
	% Authors
 	Sarah Kettell,\textsuperscript{\rm 1}
	Jacob Gallucci,\textsuperscript{\rm 2}
	Richard Bowser,\textsuperscript{\rm 3}
	Christian Overton\textsuperscript{\rm 4}
	\\
}
\affiliations {
	% Affiliations
	The Pennsylvania State University\\
	sxk1552@psu.edu,\textsuperscript{\rm 1} jag6248@psu.edu,\textsuperscript{\rm 2}  rjb5973@psu.edu,\textsuperscript{\rm 3} cto5068@psu.edu\textsuperscript{\rm 4}
}
...
\noindent Our purpose was to provide a Gin Rummy agent that was more advanced than a purely heuristic-based one. We decided to do this because implementing an agent that can train itself is an area not too thoroughly researched.

Since Gin Rummy is an imperfect information game, or a game in which the agent does not know everything that is going on (such as what is in the opponent's hand), our agent uses a combination of heuristics, Shapley value regression, and counterfactual regret minimization to make decisions when drawing, discarding, knocking, and melding cards. We combine all of these strategies together to create an agent that can be trained in a reasonable amount of time without requiring a large amount of storage.

[TODO Preliminary Results go here]
