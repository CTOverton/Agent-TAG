...
public class Sxk1552GinRummyPlayerV1 implements GinRummyPlayer {
    ...
    public ArrayList<ArrayList<Card>> getFinalMelds() {
        ...
        if(!opponentKnocked) {
            int bestDeadwood = Helper.getBestDeadwood(cards);
            ...
            if(bestDeadwood < 5 && GameState.numFaceDownCards < 10 && (bestDeadwood - Helper.getAvgDeadwoodAfterDraw(cards)) < 2) {
                return bestMeldSets.get(random.nextInt(bestMeldSets.size()));
            }
            // else, do not knock if deadwood is > 2
            if (bestDeadwood > 2) {
                return null;
            }
            // if deadwood is <= 2, do not meld if there is a reasonable chance to go gin
            if(Helper.getUnmeldableCardsAfterDraw(cards).isEmpty() && (Helper.getNumCardsForGin(cards)/(double) GameState.numFaceDownCards) > 0.2){
                return null;
            }
        }
    ...
    static class Helper {
        ...
        public static int getDeadwoodAfterDiscard(ArrayList<Card> myCards) {
            ...
            for (Card card : myCards){
                ...
                bestDeadwoodPoints = Math.min(bestDeadwoodPoints, getBestDeadwood(GinRummyUtil.bitstringToCards(myHand)));
                myHand = myHand | 1L << card.getId();
            }
